...
/*   Updated: 2024/11/03 12:49:47 by mstaali          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	sprite_textures[0] = mlx_load_png("bonus/assets/init_state_g.png");
	if (!sprite_textures[0])
		error_mssg_2(FRAMES);
	sprite_textures[1] = mlx_load_png("bonus/assets/animate_1_g.png");
	if (!sprite_textures[1])
		error_mssg_2(FRAMES);
	sprite_textures[2] = mlx_load_png("bonus/assets/animate_2_g.png");
	if (!sprite_textures[2])
		error_mssg_2(FRAMES);
	sprite_textures[3] = mlx_load_png("bonus/assets/animate_3_g.png");
	if (!sprite_textures[3])
		error_mssg_2(FRAMES);
	sprite_textures[4] = mlx_load_png("bonus/assets/animate_4_g.png");
	if (!sprite_textures[4])
		error_mssg_2(FRAMES);
	sprite_textures[5] = mlx_load_png("bonus/assets/animate_5_g.png");
	if (!sprite_textures[5])
		error_mssg_2(FRAMES);
	sprite_textures[6] = mlx_load_png("bonus/assets/animate_6_g.png");
...
void	animation_process(t_my_mlx *mlx, double current_time)
{
	...
	if (current_time - mlx->last_frame_time >= 0.04)
	{
		if (mlx->sprite_frames[mlx->curr_frame])
		{
			...
			mlx->sprite_frames[mlx->curr_frame] = NULL;
		}
		mlx->curr_frame++;
